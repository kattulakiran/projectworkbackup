...
import com.vims.rs.dao.IDGenerator;
import com.vims.rs.dao.VehicleDTO;
import com.vims.rs.dao.VehicleRepository;
import com.vims.rs.exception.InvalidFieldException;
import com.vims.rs.model.Vehicle_details;
import com.vims.rs.service.VehicleService;
	...
	public List<Vehicle_details> searchVehicle(@PathVariable("searchterm") String searchTerm) {
	 ...
	 @ApiOperation(value = "Create a new Vehicle.", response = Vehicle_details.class)
	    ...
	    public Vehicle_details createVehicle(@RequestBody Vehicle_details vehicle) {
	        ...
	        if(!((vehicle.getVehicle_Owner().matches("[a-zA-Z]{1,}[ ]{1,}.*")) || (vehicle.getVehicle_Owner().matches("[a-zA-Z]{1,}")))){
	        	throw new InvalidFieldException("Vehicle Owner");
	        }
	        
	        if(!((vehicle.getVehicle_Registration_State().matches("[a-zA-Z]{1,}")) || (vehicle.getVehicle_Registration_State().matches("[a-zA-Z]{1,}[ ]{1,}.*")))){
	        	throw new InvalidFieldException("Registration state");
	        }
	        
	        if(!(vehicle.getVehicle_Type().matches("[a-zA-Z]{1,}"))){
	        	throw new InvalidFieldException("Vehicle Type");
	        }
	        
	        if(!((vehicle.getManufacturer().matches("[a-zA-Z]{1,}")) || (vehicle.getManufacturer().matches("[a-zA-Z]{1,}[ ]{1,}.*")))){
	        	throw new InvalidFieldException("Manufacturer");
	        }
	        
	        if(vehicle.getModel()==""){
	        	throw new InvalidFieldException("Vehicle Model");
	        }
	        
	        if(!(vehicle.getVehicle_Class().matches("[a-zA-Z]{1,}"))){
	        	throw new InvalidFieldException("Vehicle Class");
	        }
	        
	        if(vehicle.getEngine_Number()==""){
	        	throw new InvalidFieldException("Vehicle Engine Number");
	        }
	        
	        if(vehicle.getMake_Year()==0){
	        	throw new InvalidFieldException("Make Year");
	        }
	        
	        if(!(vehicle.getRegistering_location().matches("[a-zA-Z]{1,}"))){
	        	throw new InvalidFieldException("Registering Location");
	        }
	        
	        if(vehicle.getPrice()==0){
	        	throw new InvalidFieldException("Vehicle Price");
	        }
	        ...
	        if(vehicle.getVehicle_Type().equalsIgnoreCase("TW")){
	        	IDGenerator.key="TW";
	        }else if(vehicle.getVehicle_Type().equalsIgnoreCase("FW")){
	        	IDGenerator.key="FW";
	        ...
	        vehicle.setPremium_amount((vehicle.getPrice()*(0.065))/12);
	        vehicleDto.setPremium_amount(vehicle.getPremium_amount());
	        }else{
	        	 vehicle.setPremium_amount((vehicle.getPrice()*(0.065)+(0.034*(vehicle.getPrice())))/12);
	        	 vehicleDto.setPremium_amount(vehicle.getPremium_amount());
	        }
	        
	        vehicle.setPolicy_id(IDGenerator.generator());
	        vehicleDto.setPolicy_id(vehicle.getPolicy_id());
	 ...
	 @ApiOperation(value = "Update the Vehicle.", response = Vehicle_details.class)
	    ...
	    public Vehicle_details updateVehicle(@PathVariable("searchterm") String searchTerm,@RequestBody Vehicle_details vehicle) {
	       ...
	       for(VehicleDTO vehicleDto : vehicleRep.findByPolicy_id(searchTerm)){ 
	    	   ...
	    	   if(!(searchTerm.equalsIgnoreCase(vehicle.getPolicy_id()))){
	    		   throw new InvalidFieldException(" Mismatch Policy id");
	    	   }
